  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      sections.forEach(section => {
        ...
        if (window.pageYOffset >= sectionTop - sectionHeight / 3) { //pageYOffset deprecated, gotta fix it later
    ...
    <nav className="fixed top-0 w-full z-50 bg-transparent">
      <div className="container mx-auto px-4">
        <div className="flex justify-end items-center h-16">
          {/* Desktop Menu */}
          <div className="hidden md:flex space-x-2 bg-white/30 backdrop-blur-md rounded-full shadow-lg p-1">
            {menuItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
          ...
          {/* Mobile Menu Button */} {/* Is not 100% round and is not exactly centered, gotta fix that later */}
      ...
      <div
        className={`
          absolute right-4 mt-2 w-56 bg-white/30 backdrop-blur-md 
        ...
        <div className="py-1">
          {menuItems.map((item) => (
            <a
              ...
              href={item.href}
              className={`
                px-4 py-2 flex items-center transition-all duration-300
                ${activeSection === item.href.replace('#', '') 
                  ? 'bg-blue-600 text-white' 
